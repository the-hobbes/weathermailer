...
func MakeRains() pb.Weather {
	p := pb.Weather{
		Sayings: []*pb.Weather_FolksySaying{
			{Saying: "it’s raining pitchforks and barns shovels",
...
func MakeAtmospheres() pb.Weather {
	p := pb.Weather{
		Sayings: []*pb.Weather_FolksySaying{
			{Saying: "it’s as thick as marsh mud", Kind: pb.Weather_ATMOSPHERE},
		},
	}
	return p
}
...
func MakeExtremes() pb.Weather {
	p := pb.Weather{
		Sayings: []*pb.Weather_FolksySaying{
			{Saying: "It blows so hard it takes two men to hold one man’s hair " +
				"on", Kind: pb.Weather_EXTREME},
			{Saying: "capful of wind", Kind: pb.Weather_EXTREME},
		},
	}
	return p
}
...
func MakeHots() pb.Weather {
	p := pb.Weather{
		Sayings: []*pb.Weather_FolksySaying{
			...
			{Saying: "Hotter’n love in hayin’ time", Kind: pb.Weather_HOT},
			{Saying: "It’s some hot today", Kind: pb.Weather_HOT},
			{Saying: "Hotter than a Methodist hell", Kind: pb.Weather_HOT},
		},
	}
	return p
}
